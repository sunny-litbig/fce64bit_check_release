...
DxB_ERR_CODE TCC_DxB_TUNER_Close(DxBInterface *hInterface, UINT32 ulTunerID)
{
    ...
    if((hInterface == NULL) || (hInterface->pTunerPrivate == NULL))
    {
        DEBUG(DEB_LEV_TCC_ERR, "[%s] Error hInterface(%ld), hInterface->pTunerPrivate(%ld)", __func__, hInterface, hInterface->pTunerPrivate);
        return DxB_ERR_ERROR;
    }
    ...
    {
        DEBUG(DEB_LEV_TCC_ERR, "[%s] Error hInterface->pOpenMaxIL is NULL");
        return DxB_ERR_ERROR;
    }
