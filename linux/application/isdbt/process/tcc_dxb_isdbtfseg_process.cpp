...
void TCC_DXB_ISDBTFSEG_PROCESS_Prepare(void *Data)
{
    ...
    {
        pCmd->iRet = pPlayer->prepare(pCmd->iSpecificInfo);
	    printf("[%s][YSKIM] Do prepare %d, result = %d\n\n", __func__, tmp_cnt, pCmd->iRet);
        ...
        {
            int ret_temp;
            ret_temp = pPlayer->release();
	        printf("[%s][YSKIM] Do release because prepare fail. (%d), result = %d\n\n", __func__, tmp_cnt, ret_temp);
        }
    }
#endif
