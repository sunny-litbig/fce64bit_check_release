...
int ISDBTPlayer::prepare(unsigned int specific_info)
{
	...
	ALOGE("[%s] start / specific_info : 0x%x, mThreadState = %d\n", __func__, specific_info, mThreadState);

	while(mThreadState != STOP)
		usleep(1000);
	mThreadState = REQUEST_RUN;

    ALOGE("[%s][YSKIM] check 1.\n", __func__);

	tcc_uithread_create("ISDBTPlayerUIThread", UI_ThreadFunc, (void *) this, getuid());

    ALOGE("[%s][YSKIM] check 2.\n", __func__);
    ...
    ALOGE("[%s][YSKIM] check 3(%d).\n", __func__, mThreadState);
...
int ISDBTPlayer::release()
{
    ...
    {
		ALOGE("[%s] TCCDxBProc_DeInit Success(%d)\n", __func__, err);
    }
	
	err = CloseDB_Table();
	if (err != 0 /*SQLITE_OK*/)
	{
		ALOGE("[%s] CloseDB_Table Error(%d)\n", __func__, err);
		//return -2;
	}
    else
	{
		ALOGE("[%s] CloseDB_Table Success(%d)\n", __func__, err);
		//return -2;
	}
